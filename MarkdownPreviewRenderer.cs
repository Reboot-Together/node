...
public static class MarkdownPreviewRenderer
{
    ...
    private static string HtmlShell(string body, IReadOnlyList<string> editableSections)
    {
        var editableJson = JsonSerializer.Serialize(editableSections);
        return $$"""
        <!doctype html>
        <html>
        <head>
          ...
          <style>
            *{box-sizing:border-box}html{background:#fff}
            ...
            h1{font-size:25px;line-height:1.28;margin:0 0 22px}h2{font-size:19px;line-height:1.38;margin:34px 0 12px;padding-bottom:8px;border-bottom:1px solid #e5e7eb}h3{font-size:16px;line-height:1.45;margin:26px 0 9px}h4{font-size:14px;margin:21px 0 7px}
            ...
            table{border-collapse:collapse;width:100%;margin:18px 0 24px;font-size:13px}thead{border-bottom:1px solid #cfd3d8}th,td{padding:9px 10px;text-align:left;vertical-align:top;border-bottom:1px solid #e5e7eb}th{color:#18181b;font-weight:700;background:#fafafa}tbody tr:hover{background:#fafafa}
        ...
        <body>
          ...
          <script>
            (() => {
              ...
              const sectionSources = {{editableJson}};
              ...
              const commitEditor = () => {
                if (!activeEditor || !activeSection) return;
                window.chrome.webview.postMessage({ type: 'update-section', index: Number(activeSection.dataset.sectionIndex), markdown: activeEditor.value });
                activeEditor = null;
                activeSection = null;
              };
