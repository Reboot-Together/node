...
public sealed class BuiltInGuideService
{
    ...
    [
        ...
        자료는 사용자가 선택한 로컬 저장소에 일반 파일로 남습니다. Markdown과 TXT는 직접 편집할 수 있고, PDF처럼 보기 전용인 자료는 원본을 바꾸지 않고 앱 안에서 읽습니다. 자동 관계 추천을 위한 의미 분석도 PC 안에서 실행되며, 원문을 외부 AI 서버로 보내지 않습니다.
        ...
        Asterism은 로컬 폴더의 자료를 정리·열람하고 관계를 나만의 지식 성좌로 탐색하는 앱입니다. Markdown과 TXT는 편집·미리보기·링크·성좌까지 지원하며, PDF는 탐색기에서 함께 관리하고 앱 안에서 읽을 수 있습니다.

        ## 기본 흐름

        1. 왼쪽 자료 탐색기에서 폴더와 자료를 정리합니다.
        2. Markdown 또는 TXT 자료를 선택해 아래 편집기에 작성합니다.
        3. 위 미리보기에서 결과를 확인하고 `[[다른 자료 제목]]`으로 관계를 만듭니다.
        ...
        | TXT `.txt` | 편집, Markdown 호환 미리보기, 내부 링크, 로컬 의미 분석, 성좌 지도 |
        ...
        모든 자료는 성좌의 후보입니다. PDF 같은 보기 전용 자료는 원문에 `[[링크]]`를 쓰지 않는 대신, 원문을 바꾸지 않는 관계 메타데이터와 추출 텍스트를 통해 검색·관계 분석·성좌 참여를 추가합니다. 현재 PDF는 뷰어와 탐색기 관리가 먼저 제공되며, 그 관계 레이어를 다음 단계로 확장합니다.
        ...
        Markdown 또는 TXT에 `[[자료 제목]]`을 입력하면 해당 자료로 이동하는 내부 링크가 만들어집니다. `[[자료 제목|표시할 이름]]`처럼 별칭도 사용할 수 있습니다.

        ## 백링크

        `성좌 지도`의 **연결 정보**를 펼치면 현재 텍스트 자료로 연결되는 문서를 볼 수 있습니다.

        ## 발견된 성좌

        로컬 임베딩 모델이 내용이 비슷한 Markdown·TXT 자료를 PC 안에서 찾아 추천합니다. `＋ 링크`를 누르기 전까지는 실제 텍스트 링크로 저장되지 않습니다. PDF도 자료 모델에서 제외하지 않으며, 원문을 바꾸지 않는 관계 메타데이터와 텍스트 추출을 기반으로 참여 범위를 확장합니다.

        ## 성좌 지도 조작

        문서 상단의 `성좌 지도` 탭이나 `Ctrl + G`를 누르면 현재 텍스트 자료를 중심으로 한 지도가 작업공간에 열립니다.
